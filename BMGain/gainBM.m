function [log10BM, snp_ab, x0, x1, rRisk] = gainBM( ct0, ct1, actions, dp, titlePrfx, tailsMethod)

%AK, 17 Sep, 2012, ouptput changed to log10BM and titlePrfx added
%AK, 9 feb, 2012,  added samll substitute for 0 in computation ct0 and ct1
%       improves prevalave map, 0,0 goes to 1 for rr mapping.
%9 Feb, 2013 Ak Added new output variable rRisk - sorted relative risk for
%genotype calls
...
EPS = 1e-8;  % added by AK on 9 feb, small constant for division
% ct0(ct0==0) = EPS;% added by AK on 9 feb, small constant for division
% ct1(ct1==0) = EPS;% added by AK on 9 feb, small constant for division

if(~exist('tailsMethod', 'var'))
    tailsMethod='binomDirect';
end
...
%if(strcmp(tailsMethod, 'normalApprox'))
    dp0=[];
%end
...
if isempty(dp0) |   dp0 ~=dp  | t0 ~= t0_0 | t1 ~= t1_0
  %  ps = [0:0.001:0.1 0.1:0.005: 0.2 0.2:0.01:0.8 0.8:0.005: 0.9 0.9:0.001:1.0]';
    ...
    ps = unique(ps);
    table0  = tailsTable(tailsMethod ,'Low',ps, [], t0, [], 1);
    table1  = tailsTable(tailsMethod ,'Up' ,ps, [], t1, [], 1);
...
ord=double(prevPerm2Idx(getPrevPerm(reshape(ct0, 1, []), reshape(ct1, 1, []))));

p0=max(EPS, ct_ab(:,1)/t0);
p1=max(EPS, ct_ab(:,2)/t1);
risk_ab  = p1./p0;
[ rRisk, ord1] = sort(risk_ab, 'descend');
assert(sum(ord-ord1')==0)
